...
- Fetch the pestphp testing dependency by executing following command on root of the project.
```sh 
  composer install 
```
- Check the pull requests tab to ensure that the bug doesn't have a fix in progress.
...
- Create and test the feature/issue and make sure it doesn't affect codebase by running full tests by executing following console command for two different environment variables.
  ...
  Run following command to make sure workflow is not affected by the change.
```sh 
  vendor/bin/pest
```
